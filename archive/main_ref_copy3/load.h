...
bool loadOBJ(const std::string &filename, std::vector<Triangle> &tris, intersectable_list &scene) {
...
bool loadSpheres(const std::string &filename, std::vector<Sphere> &sph, intersectable_list &scene) {
  int scene_size_0 = scene.size();
  std::ifstream in(filename);
  if(!in) {
    std::cerr << "Cannot open OBJ file: " << filename << "\n";
