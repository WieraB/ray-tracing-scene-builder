...
std::vector<Sphere> &spheres, std::vector<Triangle> triangles, intersectable_list &scene) {
  double inf=t=1e20;
  isTri=false;
  double d;
  ...
  for (int i=0; i<(int)scene.size(); i++) {
    Eigen::Vector3d n_tmp;
    if ((d=scene[i] -> intersect(r, n_tmp)) && d<t) {
       t=d; id=i; isTri=true; n=n_tmp;
    }
  }
...
Eigen::Vector3d radiance(const Ray &r, int depth, unsigned short *Xi, intersectable_list &scene){
  ...
  if (!intersect(r, t, id, isTri, n, spheres, triangles, scene)) return Eigen::Vector3d(0, 0, 0); // if miss, return black
