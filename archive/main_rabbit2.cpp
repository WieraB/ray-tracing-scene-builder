...
Sphere spheres[] = {//Scene: radius, position, emission, color, material
  Sphere(600, Eigen::Vector3d(-0.03, 681.6-.27, 0.014), Eigen::Vector3d(12,12,12), Eigen::Vector3d(0, 0, 0),     DIFF), //Light
  Sphere(0.12, Eigen::Vector3d(-0.03, -0.1, 0.014),    Eigen::Vector3d(0, 0, 0),  Eigen::Vector3d(1.0, 0.078, 0.576),  DIFF),//Glass
  // Sphere(80, Eigen::Vector3d(-0.03, -0.1, -100),    Eigen::Vector3d(0, 0, 0),  Eigen::Vector3d(.75,.75,.75), DIFF),//Back
};
...
void scaleAndCenterOBJ(std::vector<Triangle> &tris, double targetSize = 1.0, bool center = true) {
    if (tris.empty()) return;

    // Compute bounding box of all vertices
    Eigen::Vector3d minB( std::numeric_limits<double>::max(),
                          std::numeric_limits<double>::max(),
                          std::numeric_limits<double>::max() );
    Eigen::Vector3d maxB(-std::numeric_limits<double>::max(),
                         -std::numeric_limits<double>::max(),
                         -std::numeric_limits<double>::max() );
    ...
    Eigen::Vector3d centerB = 0.5 * (minB + maxB);
    Eigen::Vector3d sizeB   = maxB - minB;
    double maxExtent = sizeB.maxCoeff();

    // Compute scale factor so largest dimension = targetSize
    double scale = (maxExtent > 0) ? (targetSize / maxExtent) : 1.0;

    // Apply scaling and optional centering
    for (auto &tri : tris) {
        tri.v0 = (tri.v0 - (center ? centerB : Eigen::Vector3d::Zero())) * scale;
        tri.v1 = (tri.v1 - (center ? centerB : Eigen::Vector3d::Zero())) * scale;
        tri.v2 = (tri.v2 - (center ? centerB : Eigen::Vector3d::Zero())) * scale;
    }

    std::cerr << "Scaled OBJ: max extent was " << maxExtent
              << ", scaled by " << scale << "\n";
}
...
int main(int argc,char *argv[]){
  ...
  if(argc >= 2) {
    loadOBJ(argv[1], triangles);
    scaleAndCenterOBJ(triangles, 1.0, true);
  }
  ...
  Eigen::Vector3d viewDir = Eigen::Vector3d(1, 1, 1).normalized();
  double distance = 0.5;  // ~5 × radius
  Eigen::Vector3d camPos = Eigen::Vector3d(-0.03, -0.1, 0.014) + viewDir * distance;

  Eigen::Vector3d camDir = (Eigen::Vector3d(-0.03, -0.1, 0.014) - camPos).normalized(); // look at origin


  // Ensure we have a good up vector
  Eigen::Vector3d worldUp(0, 1, 0);
  if (std::abs(camDir.dot(worldUp)) > 0.99)
      worldUp = Eigen::Vector3d(0, 0, 1);  // avoid degeneracy if looking along Y axis
  ...
  Eigen::Vector3d right = camDir.cross(worldUp).normalized();
  Eigen::Vector3d up = right.cross(camDir).normalized();
