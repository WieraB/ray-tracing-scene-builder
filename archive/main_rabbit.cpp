...
Sphere spheres[] = {//Scene: radius, position, emission, color, material
  ...
  // Sphere(0.12, Eigen::Vector3d(-0.03, -0.1, 0.014),    Eigen::Vector3d(0, 0, 0),  Eigen::Vector3d(1,1,1)*.999,  REFR),//Glass
...
int main(int argc,char *argv[]){
  ...
  double fov =0.5;  // Vertical view angle (field of view)
  auto theta = degrees_to_radians(fov);
  auto scale = std::tan(theta/2);
  double aspect_ratio = double(w) / double(h);

  // Eigen::Vector3d camPos(85, 106, 85);
  Eigen::Vector3d camPos(70, 100, 70);

  // Eigen::Vector3d camPos(68.6, 100, 70);

  // Eigen::Vector3d camPos(1, 1, 1);

  //Eigen::Vector3d camPos(-0.08, 0.13, 0.052+10);
  // Eigen::Vector3d camPos(-0.08, 0.13, 70);

  Eigen::Vector3d camDir = (Eigen::Vector3d(0.0168008, -0.11015295, 0.0014823) - camPos).normalized(); // look at origin
