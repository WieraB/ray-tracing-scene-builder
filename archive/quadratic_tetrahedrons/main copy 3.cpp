    ...
    double intersect(const Ray &r, Eigen::Vector3d &n_out) const {
      ...
      double min_t = 1e20;
        ...
        for (int f = 0; f < 4; ++f) {
            ...
            Eigen::Vector3d plane_n = (v1 - v0).cross(v2 - v0).normalized();
            double denom = plane_n.dot(r.d);
            if (std::abs(denom) < eps) continue;

            double t_guess = plane_n.dot(v0 - r.o) / denom;
            
            
            
            if (t_guess < 0) continue;

            // 2. Start Newton-Raphson at t_guess, and gh from the planar projection
