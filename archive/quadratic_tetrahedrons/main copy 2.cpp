...
//                 Eigen::Matrix3d J = calc_jacobians(uvw.x(), uvw.y(), uvw.z());
//                 Eigen::Vector3d dPdu = J.col(0);
//                 Eigen::Vector3d dPdv = J.col(1);
//                 Eigen::Vector3d dPdw = J.col(2);


//                 if (std::abs(uvw.x()) < 1e-4) n_out = dPdv.cross(dPdw).normalized();
//                 else if (std::abs(uvw.y()) < 1e-4) n_out = dPdw.cross(dPdu).normalized();
//                 else if (std::abs(uvw.z()) < 1e-4) n_out = dPdu.cross(dPdv).normalized();
//                 else n_out = (dPdu - dPdw).cross(dPdv - dPdw).normalized();
...
struct Quadratic_tet {
    ...
    // Node Map for the 4 faces (each face has 6 nodes: 3 corners, 3 mid-edges)
    // Face indices based on standard 10-node tet numbering
    ...
    // w = 1 - u - v
    static Eigen::VectorXd get_face_N(double u, double v) {
        double w = 1.0 - u - v;
        Eigen::VectorXd N(6);
        N << w*(2*w-1), u*(2*u-1), v*(2*v-1), 4*u*w, 4*u*v, 4*v*w;
        return N;
    }

    static Eigen::Matrix<double, 3, 2> get_face_Jacobian(double u, double v, const std::vector<Eigen::Vector3d>& f_nodes) {
        double w = 1.0 - u - v;
        // Derivatives of N w.r.t u and v
        double dNdu[6] = { -(4*w-1), 4*u-1, 0, 4*(w-u), 4*v, -4*v };
        double dNdv[6] = { -(4*w-1), 0, 4*v-1, -4*u, 4*u, 4*(w-v) };
    ...
    double intersect(const Ray &r, Eigen::Vector3d &n_out) const {
        ...
        for (int f = 0; f < 4; ++f) {
            ...
            // 1. Initial Guess: Intersect ray with the plane of the 3 corner nodes
            // This provides a much better starting point for Newton
            Eigen::Vector3d p0 = f_nodes[0], p1 = f_nodes[1], p2 = f_nodes[2];
            Eigen::Vector3d plane_n = (p1 - p0).cross(p2 - p0).normalized();
            double denom = plane_n.dot(r.d);
            if (std::abs(denom) < 1e-8) continue;

            double t_guess = plane_n.dot(p0 - r.o) / denom;
            if (t_guess < 0) continue;

            // Start Newton at t_guess, and uv from the planar projection
            Eigen::Vector2d uv(0.33, 0.33); 
            double t = t_guess;

            for (int iter = 0; iter < 8; ++iter) {
                Eigen::VectorXd N = get_face_N(uv.x(), uv.y());
                Eigen::Vector3d P = Eigen::Vector3d::Zero();
                for(int i=0; i<6; ++i) P += N[i] * f_nodes[i];

                Eigen::Vector3d res = r.o + t * r.d - P;
                if (res.norm() < 1e-7) {
                    if (uv.x() >= -0.01 && uv.y() >= -0.01 && (uv.x() + uv.y()) <= 1.01) {
                        if (t < min_t && t > 1e-4) {
                            min_t = t;
                            Eigen::Matrix<double, 3, 2> J = get_face_Jacobian(uv.x(), uv.y(), f_nodes);
                ...
                // Newton Step: Solve [rd, -dP/du, -dP/dv] * [dt, du, dv]^T = -res
                Eigen::Matrix<double, 3, 2> J = get_face_Jacobian(uv.x(), uv.y(), f_nodes);
                Eigen::Matrix3d M;
                M.col(0) = r.d;
                M.col(1) = -J.col(0);
