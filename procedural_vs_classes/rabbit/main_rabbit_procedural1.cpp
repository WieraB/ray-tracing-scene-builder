...
// Use EIGEN_MAKE_ALIGNED_OPERATOR_NEW for alignment in structures
struct EIGEN_ALIGN16 Ray { 
  Eigen::Vector3d o, d; 
  Ray(Eigen::Vector3d o_, Eigen::Vector3d d_) : o(o_), d(d_) {} 
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
};

struct EIGEN_ALIGN16 Sphere {
  double rad;                   // radius
...
using AlignedSphereVector = std::vector<Sphere, Eigen::aligned_allocator<Sphere>>;
using AlignedTriangleVector = std::vector<Triangle, Eigen::aligned_allocator<Triangle>>;

// Store geometry in aligned vectors
AlignedSphereVector spheres = {
  //Scene: radius, position, emission, color, material
  Sphere(600, Eigen::Vector3d(-0.03*100, 681.6-.27, 0.014*100), Eigen::Vector3d(12,12,12), Eigen::Vector3d(0, 0, 0), DIFF), // Light
  Sphere(0.12*100, Eigen::Vector3d(-0.03*100, -0.1*100, 0.014*100), Eigen::Vector3d(0, 0, 0),  Eigen::Vector3d(1,1,1)*.999, REFR), // Glass
};

AlignedTriangleVector triangles;

inline double intersect_sphere(const Ray &r, const double &rad, const Eigen::Vector3d &p) {
    Eigen::Vector3d op = p-r.o; 
    double t, eps=1e-4, b=op.dot(r.d), det=b*b-op.dot(op)+rad*rad;
    if (det<0) return 0; else det=sqrt(det);
    return (t=b-det)>eps ? t : ((t=b+det)>eps ? t : 0);
}

inline double intersect_triangle(const Ray &r, Eigen::Vector3d &n_out, const Eigen::Vector3d &v0, const Eigen::Vector3d &v1, const Eigen::Vector3d &v2) {
...
inline bool intersect(const Ray &r, double &t, int &id, bool &isTri, Eigen::Vector3d &nTri) {
  ...
  // Ensure sequential access for the contiguous array (cache-friendly)
  for (size_t i=0; i<spheres.size(); i++) { // Changed from sizeof to size()
    const Sphere &s = spheres[i];
    if ((d=intersect_sphere(r, s.rad, s.p)) && d<t) { t=d; id=i; isTri=false; }
  }
  
  for (size_t i=0; i<triangles.size(); i++) {
    const Triangle &tri = triangles[i];
    Eigen::Vector3d n_tmp;
    // Passing v0, v1, v2 explicitly (only fetching the required data from memory)
    if ((d=intersect_triangle(r, n_tmp, tri.v0, tri.v1, tri.v2)) && d<t) {
...
bool loadOBJ(const std::string &filename, AlignedTriangleVector &tris) {
...
int main(int argc,char *argv[]){
  ...
  // double fov = 0.29;  // Vertical view angle (field of view)
  double fov =34;  // Vertical view angle (field of view)
  auto theta = degrees_to_radians(fov);
  auto scale = std::tan(theta/2);
  double aspect_ratio = double(w) / double(h);

  Eigen::Vector3d camPos(100, 120, 90);
  Eigen::Vector3d camDir = (Eigen::Vector3d(-0.03*100, -0.1*100, 0.014*100) - camPos).normalized(); // look at origin
  ...
  // Using aligned vector for color buffer
