...
inline double intersect_sphere(const Ray &r, const double &rad, const Eigen::Vector3d &p) { // returns distance, 0 if nohit
...
Sphere spheres[] = {//Scene: radius, position, emission, color, material
  ...
  // Sphere(80*100, Eigen::Vector3d(-0.03*100, -0.1*100, -100*100),    Eigen::Vector3d(0, 0, 0),  Eigen::Vector3d(.75,.75,.75), DIFF),//Back
};
...
inline bool intersect(const Ray &r, double &t, int &id, bool &isTri, Eigen::Vector3d &nTri) {
  ...
  for (int i=0; i<sizeof(spheres)/sizeof(Sphere); i++) {
    if ((d=intersect_sphere(r, spheres[i].rad, spheres[i].p)) && d<t) { t=d; id=i; isTri=false; }
  }
  for (int i=0; i<(int)triangles.size(); i++) {
    Eigen::Vector3d n_tmp;
    if ((d=intersect_triangle(r, n_tmp, triangles[i].v0, triangles[i].v1, triangles[i].v2)) && d<t) {
